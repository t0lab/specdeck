...
### User Story 1 - Connect GitHub qua OAuth (Priority: P1)

Là một người tự host SpecDeck, tôi authorize SpecDeck truy cập GitHub của mình **một lần**, sau đó SpecDeck liệt kê được các repo của tôi (kể cả private) để chọn khi tạo Project. Token GitHub được giữ **hoàn toàn ở server**, không bao giờ xuống trình duyệt.
...
**Independent Test**: Bấm "Connect GitHub" → hoàn tất OAuth web flow → quay lại SpecDeck thấy trạng thái "Connected as <github-login>" và danh sách repo load được. Kiểm tra DevTools Network + JS bundle: **không** thấy access token ở client.

**Acceptance Scenarios**:

1. **Given** chưa kết nối GitHub, **When** user bấm "Connect GitHub" và authorize trên GitHub, **Then** SpecDeck lưu token server-side, hiển thị github-login đã kết nối, và list được repo của user.
...
3. **Given** user huỷ flow OAuth giữa chừng (deny), **When** quay lại SpecDeck, **Then** trạng thái vẫn "chưa kết nối", có thông báo rõ và nút thử lại — không tạo bản ghi token rác.
...
- **FR-001**: Hệ thống MUST cho user kết nối tài khoản GitHub qua OAuth web flow (authorize → callback), không yêu cầu dán token thủ công.
...
- **OAuth scope** đủ để đọc danh sách repo và clone private (mức `repo`); SpecDeck đăng ký sẵn một OAuth App với callback trỏ về domain Cloudflare Tunnel của deployment (giá trị client-id/secret là cấu hình triển khai, **secret chỉ ở backend**).
